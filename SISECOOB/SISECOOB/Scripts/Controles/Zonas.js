...
function Nuevo() {
    $('#titulo').text('Nueva Zona');
    $.ajax({
        type: 'POST',
        url: '/Zonas/Formulario',
        data: { id: 0 },
        beforeSend: function () {
            Loading("Cargando");
        },
        ...
        success: function (html) {
            $('#NuevaZona').find('.modal-body form').remove();
            $('#NuevaZona').find('.modal-body').append(html);
    ...
    if (params != null) {
        $.ajax({
            ...
            success: function (data) {
                if (data.result == true) {
                    AlertSuccess('Se ha guardado el registro exitosamente.', 'Zona');
                    buscar();
                } else {
                    AlertError(data.message, 'Zona');
                }
            },
            error: function () {
                AlertError('No se pudo guardar el registro. Intente nuevamente.', 'Zona');
            }
        });
    }
}
...
$('#tzonas').on('click', 'button[name="editar"]', function () {
    Editar($(this).val());
});
...
function Editar(id) {
    ...
    $.ajax({
        ...
        success: function (html) {
            $('#EditarZona').find('.modal-body form').remove();
            $('#EditarZona').find('.modal-body').append(html);
            $('#EditarZona').modal('show');
        },
        error: function () {
...
function Editando() {
    ...
    if (params != null) {
        $.ajax({
            ...
            complete: function () {
                Loading();
                $('#EditarZona').modal('hide');
            },
            success: function (data) {
                if (data.result == true) {
                    AlertSuccess('Se ha Actualizado el registro exitosamente.', 'Zona');
                    buscar();
                } else {
                    AlertError(data.message, 'Zona');
                }
...
$('#tzonas').on('click', 'button[name="eliminar"]', function () {
    Elimina($(this).val());
});
...
function Elimina(id) {
    $.ajax({
        ...
        success: function (data) {
            if (data.result == true) {
                AlertSuccess('Se ha Eliminado el registro exitosamente.', 'Zona');
                buscar();
            } else {
                AlertError(data.message, 'Zona');
            }
